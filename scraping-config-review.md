...
**SJ Downtown 抓取详情:**
- **API类型**: WordPress Events Calendar REST API
- **数据质量**: ⭐⭐⭐⭐⭐ (结构化数据，字段完整)
- **覆盖范围**: San Jose Downtown, SoFA District, 市中心商圈
- **活动类型**: First Fridays, 市集, 社区活动, 艺术活动
- **更新频率**: 实时API，数据最新
- **限制**: 每次最多50个 (`per_page: 50`)

#### 2. 传统爬虫 (3个)
```javascript
legacy_scrapers: [
  EventbriteScraper,    // ⚠️ 产出不足: 11个 (理论150个)
  SFStationScraper,     // ✅ 产出正常: 25个 (理论60个)
  FuncheapWeekendScraper // ❌ 产出极少: 1个 (理论50个)
]
```
...
**配置的其他数据源:**
- 📡 **REST API**: SJ Downtown (已启用) ✅
- 🌐 **CSS抓取**: 目前为空 (DoTheBay已移除)
- 🤖 **AI抓取**: 60+个季节性源 (根据月份自动过滤)
...
| 数据源 | config.js设置 | 实际代码限制 | 本周抓取 | 完成率 | 状态 |
|--------|--------------|-------------|---------|-------|------|
| Eventbrite | 100 | **150个** (3层) | **11个** | 7% | ❌ 异常 |
| SF Station | 100 | **60个** | **25个** | 42% | ✅ 正常 |
| **SJ Downtown API** | 100 | **50个** | **17个** | 34% | ✅ 正常 |
| Funcheap | 100 | **50个** | **1个** | 2% | ❌ 异常 |

**数据源质量分析:**

| 数据源 | 数据质量 | 覆盖范围 | 活动类型 | 稳定性 |
|--------|---------|---------|---------|-------|
| **SJ Downtown** | ⭐⭐⭐⭐⭐ | San Jose市中心 | First Fridays, 市集, 艺术 | 🟢 极高 (REST API) |
| SF Station | ⭐⭐⭐⭐ | 旧金山全域 | 社区活动, 志愿活动, 文化 | 🟢 高 |
| Eventbrite | ⭐⭐⭐⭐⭐ | 全湾区15+城市 | 商业活动, 付费活动, 多样化 | 🟡 中 (CSS变化) |
| Funcheap | ⭐⭐⭐ | 旧金山为主 | 免费活动, 节日, 市集 | 🔴 低 (选择器失效?) |

**关键发现:**
1. **config.js的 `maxEventsPerSource: 100` 并不是实际限制!**
2. **每个scraper有自己的硬编码限制**
3. **SJ Downtown API 是最稳定的数据源** (REST API, 数据结构化)
4. **Eventbrite和Funcheap远低于理论值** → 抓取逻辑或选择器失效
...
**当前活跃数据源 (4个):**
1. ✅ **SJ Downtown API** (REST) - 17个 - 最稳定
2. ✅ **SF Station** (HTML) - 25个 - 正常工作
3. ⚠️ **Eventbrite** (HTML) - 11个 - 产出不足
4. ❌ **Funcheap** (HTML) - 1个 - 几乎失效

**潜在可用但未启用的源:**
- 🤖 AI抓取源 (San José Made, 365 Night Market, Oakland First Fridays, SF Chinese New Year)
- 📅 固定时间活动 (First Fridays, Berryessa Night Market - 季节外)

**数据源推荐优先级:**
1. 🔴 **P0 - 立即修复**: Eventbrite (应该有80-150个)
2. 🔴 **P0 - 立即修复**: Funcheap (应该有20-50个)
3. 🟡 **P1 - 尽快启用**: AI抓取源 (4个月度/季节活动)
4. 🟢 **P2 - 长期优化**: 添加更多REST API源
